...
$(function () {
    ...
    ***************************/
    ...
    // Function to reinitialize all custom scripts after Swup replaces content
    ...
    function initSwiper() {
        ...
        swiperContainers.forEach(container => {
            new Swiper(container, {
                ...
                // Add any additional Swiper parameters here
            });
        });
    }

    // Function to reinitialize all custom scripts after Swup replaces content
    function reinitCustomScripts() {
        // Reinitialize phone input logic
        ...
        if (phoneInput) {
            ...
            phoneInput.addEventListener('input', function() {
                if (!phoneInput.value.startsWith('+998')) {
                    phoneInput.value = '+998' + phoneInput.value.slice(4);
                }
            });
        ...
        // Reinitialize job location and position logic
        const jobLocationType = document.getElementById('job_location_type');
        const positionSelect = document.getElementById('position');

        const ofisPositions = [
            "Call center", "HR menejer yordamchisi", "IT bo'limi", "Marketolog",
            "Media bo'limi", "Microsoft Access dasturchisi", "Moliya bo'limi", 
            "Onlayn shartnoma rasmiylashtiruvchi", "Sifat nazorati bo'limi", 
            "SysAdmin", "Undiruv operatori", "O'qituv bo'limi"
        ];

        const filialPositions = [
            "Filial rahbari", "Kassa", "Mebel yig'uvchi", "Omborchi", 
            "Oshpaz", "Shartnoma rasmiylashtiruvchi", "Sotuv agenti", 
            "Sotuvchi maslaxatchi", "Servis", "Supervayzer", "Tozalik hodimasi", 
            "Undiruvchi", "Yuk tashuvchi", "Yuk yuklovchi"
        ];

        if (jobLocationType && positionSelect) {
            jobLocationType.addEventListener('change', function() {
                positionSelect.innerHTML = '<option value="" disabled selected hidden>Lavozimingizni tanlang</option>';
                const positions = (this.value === 'Ofis') ? ofisPositions : filialPositions;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        reinitCustomScripts(); // Call to initialize all custom scripts, including Swiper
    });

    // Swup event listener to reinitialize scripts after content is replaced
    document.addEventListener('swup:contentReplaced', function() {
        reinitCustomScripts(); // Call to reinitialize scripts after Swup content change
    });
